    ...
    public int save(Student student) {
        try {
            ...
            runner.update("insert into student(stuno,realname,pwd,phone,sex,createDate) values (?,?,?,?,?,?)",
                   student.getStuno(),student.getRealname(),student.getPwd(),student.getPhone(),student.getSex(),student.getCreateDate());
        } catch (SQLException e) {
    ...
    public int update(Student student) {//更改学生信息
        try {//执行更改
            runner.update("update student set stuno=?, realname=?,pwd=?,phone=?,sex=? where id=?",
            		 student.getStuno(),student.getRealname(),student.getPwd(),student.getPhone(),student.getSex(),student.getId());
        } catch (SQLException e) {
            throw new RuntimeException(e);//抛出运行异常
        }
        return 1;
