...
//			});
	...
	protected void updateVcd(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {//更改
		...
		String creatTime=request.getParameter("creatTime").replace("-","/");
	...
	protected void add(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {//添加
		...
		System.out.println("creatime:" + creatTime);
		String profile=request.getParameter("profile");
		Double price=Double.parseDouble(request.getParameter("price"));
		Integer stock = Integer.parseInt(request.getParameter("stock"));
...
//			String createDate=simpleDateFormat.format(nowtime);
	...
	protected void queryVcdBuyRecord(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		request.getRequestDispatcher("/WEB-INF/vcd/buyrecord.jsp").forward(request,response);
	}

	protected void queryVcdRecordBetween(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String btime = request.getParameter("beginTime").replace("-","/");
		String etime = request.getParameter("endTime").replace("-","/");

		// 查询 VCD 购买记录
		List<VcdBuyRecord> result = service.findVcdBetween(btime, etime);

		// 将结果存入请求属性
		request.setAttribute("vcdBuyRecordList", result);
	...
	protected void queryVcdBorrowRecordBetween(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String btime = request.getParameter("beginTime").replace("-","/");
		String etime = request.getParameter("endTime").replace("-","/");

		// 查询 VCD 借还记录
		List<VcdUserRecord> result = service.findBorrowVcdBetween(btime, etime);

		// 将结果存入请求属性
		request.setAttribute("vcdUseRecordList", result);
		request.setAttribute("beginTime", request.getParameter("beginTime"));
		request.setAttribute("endTime", request.getParameter("endTime"));
